...
- **内置技能**：Chat / 图像生成 / TTS / STT / 嵌入 / 网页搜索 / 网页抓取 / 视频生成，每个技能对应一个标准化 SKILL.md，可在 `仪表盘 → 技能` 页面一键复制给 Claude Code / Codex / Cursor 使用。技能内含「首次成功路径、新手错误表、Console Log 排障链路、调用前先确认 provider/模型、不自动重试计费请求、语言跟随用户」等工程化约束。
